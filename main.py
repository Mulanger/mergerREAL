...
def time_until_next_run():
    # Get current time (assume local time is Swedish time)
    now = datetime.now()
    next_run = now.replace(hour=0, minute=15, second=0, microsecond=0)

    # If we have passed today's 00:15 AM, schedule for tomorrow
    if now > next_run:
...
while True:
    ...
    print(f"Sleeping for {time_to_sleep // 3600} hours and {(time_to_sleep % 3600) // 60} minutes...")
    ...
    if should_run_today():
        ...
        print("Skipping task for today (Sunday/Monday).")
